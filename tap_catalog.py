...
class _DataCatalogPublishJson(object):
    ...
    """
    def __init__(self, target_uri, size=0, title=None, org_uuid=None, format = None, source_uri = None, is_public=False, category = None, record_count = -1, data_sample=""):
        ...
        if response.status_code == httplib.OK:
            ...
            if len(resources) == 1:
                ...
                if choice not in range(1, orgs_len+1):
                    ...
                    return filter(lambda row: row["entity"]["name"] == orgs[choice-1], resources)[0]["metadata"]["guid"]
        else:
